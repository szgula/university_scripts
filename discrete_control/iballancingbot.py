...
class IBalancingBot:
    ...
    def runge_kutta(self, deltat, F):
        ...
        k2phip, k2phipp, k2xp, k2xpp, k2psip, k2psipp = \
            self.f(self.phi + k1phip / 2, self.phip + k1phipp / 2, self.x + k1xp / 2, self.xp + k1xpp / 2,
                   self.psi + k1psip / 2, self.psip + k1psipp / 2, deltat, F)
        k3phip, k3phipp, k3xp, k3xpp, k3psip, k3psipp = \
            self.f(self.phi + k2phip / 2, self.phip + k2phipp / 2, self.x + k2xp / 2, self.xp + k2xpp / 2,
                   self.psi + k2psip / 2, self.psip + k2psipp / 2, deltat, F)
        k4phip, k4phipp, k4xp, k4xpp, k4psip, k4psipp = \
            self.f(self.phi + k3phip, self.phip + k3phipp, self.x + k3xp, self.xp + k3xpp, self.psi + k3psip,
                   self.psip + k3psipp, deltat, F)
